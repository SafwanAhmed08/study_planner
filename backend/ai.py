...
def generate_flashcards(subtopic,topic,num):
    content_chunks = retrieval(subtopic,topic)
    context = "\n\n".join(content_chunks)
    payload = {
        "messages":[
            ...
            {
                "role":"user","content":f"For this context {context},the number of flashcards to create are {num} only flash cards about {topic}"
            }
        ],
        "temperature" : 0.1,
        "stream" : False
...
def extract_prerequisites(text):
    payload = {
        "messages": [
            {
                "role": "system",
                "content": """You are a course prerequisite parser.
                Your job is to extract ONLY knowledge, skill, or background requirements from a course description.
                ...
                Each item must have:
                - "name": a short prerequisite name
                - "description": one clear sentence explaining what the student should know or be able to do
                - "category": one of "maths", "programming", "ml", "other"

                Rules:
                - Extract only academic/skill prerequisites.
                - Ignore administrative rules, registration rules, course codes, degree programme eligibility, credits, assessment format, and scheduling information.
                - If a sentence contains multiple distinct requirements, split them into separate items.
                - Do not over-split tiny subtopics if they belong under one clear prerequisite.
                - Keep names short and clean.
                - Use "maths" for algebra, calculus, probability, vectors, matrices, statistics, optimisation, etc.
                - Use "programming" for coding, frameworks, Python, PyTorch, JAX, algorithms, big-O, implementation knowledge, etc.
                - Use "ml" for machine learning concepts, model evaluation, data splitting, preprocessing, tuning, baselines, etc.
                - Use "other" only when none of the above categories fit.
                - Avoid duplicates.
                - If no prerequisites are found, return [].

                Example:
                [
                {
                    "name": "Linear Algebra",
                    "description": "Students should be comfortable with vectors, matrices, inner products, and quadratic forms.",
                    "category": "maths"
                },
                {
                    "name": "Probability",
                    "description": "Students should be able to work with random variables, simple distributions, and expectations.",
                    "category": "maths"
                },
                {
                    "name": "Machine Learning Basics",
                    "description": "Students should understand data splitting, preprocessing, model tuning, and comparison of baseline machine learning methods.",
                    "category": "ml"
                },
                {
                    "name": "Programming Fundamentals",
                    "description": "Students should be familiar with programming, basic computer science concepts, and reading short machine learning code snippets.",
                ...
                {
                    "role": "user",
                    "content": f"Extract prerequisites from this course description:\n\n{text}"
                }
            ],
            "temperature": 0.1,
            "stream": False
